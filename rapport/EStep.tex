...
\notes{expliquer leur role respectif... En fait Kalman c'est pour le filtering et l'autre pour le smoothing, c'est vrai que c'est ecrit dans leur nom mais il faut preciser que le choix depend si l'on veut faire du batch ou du online: et j'ai l'impression que l'on fait du online, quelle est donc la pertinence du Kalman? Dire aussi que l'un est un premisse a l'utilisation de l'autre}

This forward-backward approach, involving a linearization at each time step, is called Extented Kalman Smoothing (EKS). \textbf{One of the important issues is to define $\mathbf{\hat{x}_t}$.}\notes{Il faut le faire absolument}
...
As in \cite{Poly}, we denote by $\hat{x}_{t|t}$ the mean of $x_t$ conditioned on $u_1;\cdots;u_t$ and for $\hat{P}_{t|t}$ the covariance of $x_t$ given $u_1;\cdots;u_t$. It is straightforward to adapt these notations in the smoothing case ($\hat{P}_{t|T}$ and $\hat{x}_{t|T}$). Namely we have: \notes{Sous quelle distribution p? Faudra expliquer pourquoi le point $\hat{x}_t$ est bien choisi parce qu'ici il est defini a un certain moment}:
\begin{align*}
...
\end{align*}
Then, in the recursion formula, we just have \notes{pourquoi est ce que ca fonctionne de juste remplacer} to replace the matrices $A$ and $C$  by the matrices $A_t$ and $C_t$ :
\begin{align*}
